<?php
	...
	*/
	...
	$pos = isset($_REQUEST['pos']) ? (int) $_REQUEST['pos'] : null;
	$status = isset($_REQUEST['status']) ? sanitize_text_field($_REQUEST['status']) : null;
	$task = isset($_REQUEST['t']) ? sanitize_text_field($_REQUEST['t']) : null;
	...
	if($do == 'update') {
		...
		$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."sharebar SET enabled='%d', position='%d', name='%s', big='%s', small='%s' WHERE id='%d'", (int) $_POST['enabled'], (int) $_POST['position'], sanitize_text_field($_POST['name']), $_POST['big'], $_POST['small'], $id));
	}
	elseif($do == 'add') {
		check_admin_referer( 'wp_sharebar_add_update' );
		$wpdb->query($wpdb->prepare("INSERT INTO ".$wpdb->prefix."sharebar (position, name, big, small) VALUES('%d','%s', '%s', '%s')", (int) $_POST['position'], sanitize_text_field($_POST['name']), $_POST['big'], $_POST['small']));
	}
	elseif($do == 'delete') {
	...
	elseif($do == 'settings'){
		...
		$_POST['width'] = $_POST['width'] ? (int) $_POST['width']:1000;
		...
		$uptask = sanitize_text_field($_POST['update-task']);
		if($buttons){
			foreach ($buttons as $button)
				sharebar_update_button((int) $button,$uptask);
			$status = "Buttons have been ".esc_html($uptask)."d";
		}else
...
<?php if($status){?><div id="message" class="updated fade"><?php echo esc_html($status); ?></div><?php } ?>
...
<?php }elseif($task == 'settings'){ ?>
	...
	<form action="?page=<?php echo esc_attr($_GET['page']); ?>&t=settings" method="post">
...
<?php }else{ ?>
	...
	<div class="alignright" style="margin-top: 10px;">
		<a href="?page=<?php echo esc_attr($_GET['page']); ?>&t=reset" class="alignleft button right-button" style="margin-right: 10px;">Reset Buttons</a><a href="?page=Sharebar&t=new" class="button-primary alignleft right-button">Add New Button</a>
	</div>
